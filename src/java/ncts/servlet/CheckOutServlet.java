...
public class CheckOutServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<BookDTO> listOutOfStock = new ArrayList<>();
            if (cart != null) {
                Map<BookDTO, Integer> items = cart.getItems();
                for (BookDTO bookDTO : items.keySet()) {
                    BookDTO dto = bookDAO.checkQuantity(bookDTO.getIdBook(), items.get(bookDTO));
                    if (dto != null) {
                        listOutOfStock.add(dto);
                    }
                }
                if (listOutOfStock.size() > 0) {
                    url = VIEW_CART_PAGE;
                    session.setAttribute("LIST_OUT_OF_STOCK", listOutOfStock);
                } else {
                    int totalPrice = Integer.parseInt(request.getParameter("txtTotal"));
                    session.setAttribute("TOTAL", totalPrice);
                    session.removeAttribute("LIST_OUT_OF_STOCK");
                    url = CHECK_OUT_PAGE;
                }
            }
